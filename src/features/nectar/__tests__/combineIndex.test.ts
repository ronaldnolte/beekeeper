...
describe('satelliteStrength', () => {
  it('is 1 when the ground is fully green and fully watered', () => {
    expect(satelliteStrength({ vigor: 1, moisture: 1 })).toBeCloseTo(1, 6);
  });
  ...
  it('is 0 when there is no growth and no canopy water', () => {
    ...
    expect(satelliteStrength({ vigor: 0, moisture: 0.7 })).toBeCloseTo(0, 6);
  });
  ...
  it('re-expands moisture so it cannot dominate by never being small', () => {
    ...
    const dead = satelliteStrength({ vigor: 0, moisture: 0.7 });
    expect(dead).toBeLessThan(0.05);
  });

  it('lets green-but-dry and dry-but-green land in the middle', () => {
    expect(satelliteStrength({ vigor: 1, moisture: 0.7 })).toBeCloseTo(0.5, 6);
    expect(satelliteStrength({ vigor: 0, moisture: 1.0 })).toBeCloseTo(0.5, 6);
  });

  it('clamps readings outside the expected range', () => {
    expect(satelliteStrength({ vigor: 5, moisture: 5 })).toBeCloseTo(1, 6);
    expect(satelliteStrength({ vigor: -3, moisture: 0 })).toBeCloseTo(0, 6);
  });
});

describe('applySatellite', () => {
  const good = { vigor: 1, moisture: 1 };
  const bad = { vigor: 0, moisture: 0.7 };
  ...
  it('never increases a bloom above what the plant list says', () => {
    for (const v of [0, 0.3, 0.7, 1]) {
      for (const m of [0.7, 0.85, 1]) {
        expect(applySatellite(1.5, { vigor: v, moisture: m }, 0.4)).toBeLessThanOrEqual(1.5 + 1e-9);
      }
    }
  });
...
describe('combineSeries', () => {
  ...
  it('joins on date and reports the factor it used', () => {
    const out = combineSeries(bloom, [
      { date: '2026-06-01', vigor: 1, moisture: 1 },
      { date: '2026-06-02', vigor: 0, moisture: 0.7 },
    ], 0.4);
  ...
  it('leaves days with no satellite reading untouched', () => {
    ...
    const out = combineSeries(bloom, [{ date: '2026-06-01', vigor: 1, moisture: 1 }], 0.4);
  ...
  it('always keeps the untouched bloom value alongside the combined one', () => {
    const out = combineSeries(bloom, [{ date: '2026-06-01', vigor: 0, moisture: 0.7 }], 0);
